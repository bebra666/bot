...
def subcategory(message):
    ...
    global users
    try:
        #print(users[message.chat.id].flag, users[message.chat.id].week, users[message.chat.id].day)
        """
        print(users[message.chat.id].flag, users[message.chat.id].data, users[message.chat.id].flag_data,
              users[message.chat.id].week, users[message.chat.id].day, users[message.chat.id].teacher_letter,
              users[message.chat.id].teacher, users[message.chat.id].class_num, users[message.chat.id].class_num_letter,
              users[message.chat.id].cabinet, users[message.chat.id].floor)"""
    ...
    #класс
            ...
            if day_check(message.text):
                users[message.chat.id].choose_day(message.text)
                users[message.chat.id].reflag('choose')
                #print(users[message.chat.id].week, users[message.chat.id].class_num_letter, users[message.chat.id].day)
                bot.send_message(message.chat.id, clas_s(users[message.chat.id].week, users[message.chat.id].class_num_letter, users[message.chat.id].day))
                bot.send_message(message.chat.id, 'Какое расписание еще показать?', reply_markup=keyboard_category)
            else:
                day_choose(message)
