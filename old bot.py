...
def subcategory(message):
    ...
    global users
    try:
        #print('1', users[message.chat.id].flag, message.text)
        if message.text == "На учителя" and users[message.chat.id].flag == 'choose':
            ...
            if users[message.chat.id].data[0] == "teacher":
                ...
                bot.send_message(message.chat.id, clas_s(users[message.chat.id].week,
                                                         str(users[message.chat.id].class_num) + str(users[message.chat.id].class_num_letter), users[message.chat.id].day))
            elif users[message.chat.id].data[0] == "cab":
                users[message.chat.id].choose_cabinet(users[message.chat.id].data[1])
                tomorrow_set(message)
                bot.send_message(message.chat.id,
                                 cabinet.timelist(users[message.chat.id].week, users[message.chat.id].day,
                                                  users[message.chat.id].cabinet))
    ...
    #учитель
            ...
            if day_check(message.text):
                ...
                bot.send_message(message.chat.id, teacher(users[users[message.chat.id].week, message.chat.id].teacher, users[message.chat.id].day))
                bot.send_message(message.chat.id, 'Какое расписание еще показать?', reply_markup=keyboard_category)
                users[message.chat.id].reflag('choose')
            else:
                day_choose(message)
    ...
    #класс
            ...
            if message.text == 'Четная':
                ...
                for day in ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота']:
                    bot.send_message(message.chat.id, clas_s(users[message.chat.id].week,
                                                             str(users[message.chat.id].class_num) + str(users[message.chat.id].class_num_letter), day), reply_markup=None)
                bot.send_message(message.chat.id, 'Какое расписание еще показать?', reply_markup=keyboard_category)
                users[message.chat.id].reflag('choose')
            elif message.text == 'На всю нечетную неделю':
                users[message.chat.id].choose_week('Нечетная')
    ...
    #кабинет
            ...
            if message.text == 'Четная':
                ...
                users[message.chat.id].choose_week('Четная')
                for day in ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота']:
                    bot.send_message(message.chat.id, cabinet.timelist(users[message.chat.id].week, day,
                                                      users[message.chat.id].cabinet), reply_markup=None)
                bot.send_message(message.chat.id, 'Какое расписание еще показать?', reply_markup=keyboard_category)
                users[message.chat.id].reflag('choose')
            elif message.text == 'На всю нечетную неделю':
                users[message.chat.id].choose_week('Нечетная')
    ...
    #print('2', users[message.chat.id].flag)
